...
const Container = props => {
  ...
  const className = classnames(sliderClass('_', vertical && 'vertical', props.disabled && 'disabled'), props.className)
  ...
  return (
    <div style={newStyle} className={className}>
      ...
      <div ref={bindElement} onClick={handleClick} className={sliderClass('inner')}>
        {range && (
          <Slider {...other} index={0} max={value[1]} onChange={handleChange} value={value[0]} vertical={vertical} />
        )}

        <Slider {...other} index={1} min={value[0]} onChange={handleChange} value={value[1]} vertical={vertical} />
      </div>
      {renderScale()}
    </div>
  )
